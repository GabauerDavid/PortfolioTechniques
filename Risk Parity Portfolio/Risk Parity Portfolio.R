...
RiskParityPortfolio = function(data, H) {
   k = ncol(data)
   n = nrow(data)
   ...
   for (i in 1:n) {
      port = riskParityPortfolio::riskParityPortfolio(H[,,i])$w
      port = ifelse(port>1,1,port)
      port = ifelse(port<0,0,port)
      portfolio_weights[i,] = port/sum(port)
   }
...
rpp = RiskParityPortfolio(data, H)
rpp$summary
rpp$hedging_effectiveness # variance reduction (Ederington, 1979)
rpp$pvalue # significance level (alpha), (Antonakakis et al., 2018)

par(mfcol = c(ceiling(k/2), 2), oma = c(.5,.5,0,0) + 0.1, mar = c(.5,.5,0,0) + 1, mgp=c(.5, .5, 0))
for (i in 1:k) {
   plot(date, rpp$portfolio_weights[,i], type="l", col="steelblue4", las=1, ylim=c(0, max(rpp$portfolio_weights)), main=paste(colnames(data)[i], "weight"), xlab="", ylab="", xaxs="i", yaxs="i")
   grid(); box()
   lines(date, rpp$portfolio_weights[,i], col="steelblue4")
}

par(mfrow=c(1,1))
plot(date, rpp$cumulative_portfolio_return, type="l", col="steelblue4", las=1, ylim=c(min(rpp$cumulative_asset_return), max(rpp$cumulative_asset_return)), main="", xlab="", ylab="", xaxs="i", yaxs="i")
for (i in 1:k) {
   lines(date, rpp$cumulative_asset_return[,i], col="grey", lty=i)
   grid(); box()
   abline(h=0, lty=3)
}
...
apply(rpp$portfolio_return, 2, mean) / apply(rpp$portfolio_return, 2, sd)
