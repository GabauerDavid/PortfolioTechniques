...
### BROADSTOCK, D., CHATZIANTONIOU, I., AND GABAUER, D. (2019)
### PATTERNS OF RETURN TRANSMISSION AND DYNAMIC PORTFOLIO WEIGHTS BETWEEN GREEN AND BLACK BONDS: NEW EVIDENCE FROM CHINA, EUROPE AND THE US
...
MinimumConnectednessPortfolio = function(data, H, type="long") {
  k = ncol(data)
  n = nrow(data)
  I = matrix(1,k,1)
...
for (i in 1:length(TICKERS)) {
  data = cbind(data, diff(log(get(TICKERS[i])[,1])))
}
data = na.omit(data)
date = index(data)
...
for (i in 1:k) {
  for (j in 1:k) {
    PCI[i,j,] = 2 * (CV[i,j,] + CV[j,i,]) / (CV[i,i,] + CV[i,j,] + CV[j,i,] + CV[j,j,])
  }
}
...
pcp = MinimumConnectednessPortfolio(data=Y/100, H=PCI, type="long")
pcp$summary
pcp$hedging_effectiveness # variance reduction (Ederington, 1979)
pcp$pvalue # significance level (alpha), (Antonakakis et al., 2018)

par(mfcol = c(ceiling(k/2), 2), oma = c(.5,.5,0,0) + 0.1, mar = c(.5,.5,0,0) + 1, mgp=c(.5, .5, 0))
for (i in 1:k) {
   plot(date, pcp$portfolio_weights[,i], type="l", col="steelblue4", las=1, ylim=c(0, max(pcp$portfolio_weights)), main=paste(colnames(data)[i], "weight"), xlab="", ylab="", xaxs="i", yaxs="i")
   grid(); box()
   lines(date, pcp$portfolio_weights[,i], col="steelblue4")
}

par(mfrow=c(1,1))
plot(date, pcp$cumulative_portfolio_return, type="l", col="steelblue4", las=1, ylim=c(min(pcp$cumulative_asset_return), max(pcp$cumulative_asset_return)), main="", xlab="", ylab="", xaxs="i", yaxs="i")
for (i in 1:k) {
   lines(date, pcp$cumulative_asset_return[,i], col="grey", lty=i)
   grid(); box()
   abline(h=0, lty=3)
}
...
apply(pcp$portfolio_return, 2, mean) / apply(pcp$portfolio_return, 2, sd)
